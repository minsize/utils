...
/**
 * Функция по умолчанию для обработки токенов
 * Возвращает токен без изменений
 */
const defaultOnToken = (token: TextToken): TextToken => token
...
const parseTextTokens = (
  ...
  const urlRegex =
    customRegex ||
    (requireProtocol ? URL_REGEX_WITH_PROTOCOL : DEFAULT_URL_REGEX)
  const ast: TextToken[] = []
  ...
  /**
   * Обрабатывает текст на поиск эмодзи
   */
  const processTextForEmojis = (text: string): void => {
    if (!text) return

    const emojiMatches = [...text.matchAll(EMOJI_REGEX)]

    if (emojiMatches.length === 0) {
      ast.push(onToken({ type: "raw", value: text }))
      return
    }
    ...
    for (const match of emojiMatches) {
      ...
      if (emojiIndex > lastIndex) {
        const textBefore = text.substring(lastIndex, emojiIndex)
        ast.push(onToken({ type: "raw", value: textBefore }))
      }

      // Добавляем эмодзи
      ast.push(onToken({ type: "emoji", value: emoji }))
    ...
    if (lastIndex < text.length) {
      ...
      ast.push(onToken({ type: "raw", value: remainingText }))
    }
  }
  ...
  let lastUrlIndex = 0
  let urlMatch
  const urlRegexWithG = new RegExp(urlRegex.source, "g")

  while ((urlMatch = urlRegexWithG.exec(remainingInput)) !== null) {
    const url = urlMatch[0]
    const urlIndex = urlMatch.index

    // Обрабатываем текст перед URL на наличие эмодзи
    if (urlIndex > lastUrlIndex) {
      const textBeforeUrl = remainingInput.substring(lastUrlIndex, urlIndex)
      processTextForEmojis(textBeforeUrl)
    }

    // Добавляем URL
    ast.push(onToken({ type: "url", value: url }))

    lastUrlIndex = urlIndex + url.length
  }

  // Обрабатываем оставшийся текст после последнего URL
  if (lastUrlIndex < remainingInput.length) {
    const remainingText = remainingInput.substring(lastUrlIndex)
    processTextForEmojis(remainingText)
  }

  return ast
}
export default parseTextTokens
